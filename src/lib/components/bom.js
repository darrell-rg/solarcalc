...
export let defaultCellFormat = {
	//	if a data cell does not have a format, then this one is used
	italics: 0,
	bold: 0,
	underline: 0,
	fontsize: 13,
	...
	readonly: false,
	displayFormat: 'TEXT' //	DATE,TIME, DURATION, CURRENCY2, CURRENCY, TEXT, NUMBER, NUMBER2
};
...
export function makeDefaultCell(value = '') {
	return {
		value: value,
		display: value, //	what is displayed often isn't the same as the value. E.G. formulas, 12=>$12.00, -12=>(ý12.00). It is the parents responsibility to do this
		result: value, //	the result of a formula or just the value as a string/number
		format: JSON.parse(JSON.stringify(defaultCellFormat))
	};
}

export function makeHeaderCell(value = '') {
	let c = {
		value: value,
		display: value, //	what is displayed often isn't the same as the value. E.G. formulas, 12=>$12.00, -12=>(ý12.00). It is the parents responsibility to do this
		result: value, //	the result of a formula or just the value as a string/number
	...
	c.format.bold = true;
	c.format.readonly = true;
	c.format.fontsize = 15;
	c.format.color = '#000';
	// c.format.background ="#1100c8";
	return c;
}

export function minSelRow(config) {
	return Math.min(config.selCells.row, config.selCells.row2);
}
export function maxSelRow(config) {
	return Math.max(config.selCells.row, config.selCells.row2);
}
export function minSelCol(config) {
	return Math.min(config.selCells.col, config.selCells.col2);
}
export function maxSelCol(config) {
	return Math.max(config.selCells.col, config.selCells.col2);
}
...
// 			if (!config.columnSetting[c]) {
// 				//  copy the most recent column
...
export function defaultGridConfig(cols = 1, rows = 1) {
	let conf = {
		columnSetting: [{ resizable: true, width: 100 }],
		rowSetting: [{ resizable: true, height: 24 }],
	...
	for (let i = 1; i < rows; i++) {
		conf.rowSetting[i] = JSON.parse(JSON.stringify(conf.rowSetting[0]));
	}

	for (let i = 1; i < cols; i++) {
		conf.columnSetting[i] = JSON.parse(JSON.stringify(conf.columnSetting[0]));
	}
	return conf;
}
...
export function makeStringRef(ref) {
	...
	if (
		ref.row2 != undefined &&
		ref.col2 != undefined &&
		(ref.row2 != ref.row || ref.col2 != ref.col)
	) {
		//	and one of row & col are different
		str += ':';
...
export function parseRef(s) {
	...
	if (s[idx] >= 'A' && s[idx] <= 'Z') {
		ref.col = s.charCodeAt(idx) - 65;
		idx++;
	}
	if (s[idx] >= 'A' && s[idx] <= 'Z') {
		ref.col = 26 + ref.col * 26 + s.charCodeAt(idx) - 65;
		idx++;
	}
	if (ref.col == undefined) {
		ref.error = 'Bad column';
		return ref;
	}
	...
	if (s[idx] == ':') {
		...
		if (s[idx] >= 'A' && s[idx] <= 'Z') {
			ref.col2 = s.charCodeAt(idx) - 65;
			idx++;
		}
		if (s[idx] >= 'A' && s[idx] <= 'Z') {
			ref.col2 = 26 + ref.col2 * 26 + s.charCodeAt(idx) - 65;
			idx++;
		}
		if (ref.col2 == undefined) {
			ref.error = 'Bad column2';
			return ref;
		}
...
export const headers = {
	0: makeHeaderCell('PN'),
	1: makeHeaderCell('Link'),
	2: makeHeaderCell('Desc'),
	3: makeHeaderCell('Qty'),
	4: makeHeaderCell('Price'),
	5: makeHeaderCell('Total'),
	6: makeHeaderCell('Note')
};
