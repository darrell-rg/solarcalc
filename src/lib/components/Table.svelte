<script>
	...
	import { defaultCellFormat } from './bom.js';
	import {
		getGridCellOrNull,
		minSelCol,
		minSelRow,
	...
	$: (isVerticalScrollbar = elContentScroller?.scrollHeight != elContentScroller?.clientHeight),
		config; //, console.log('isVerticalScrollbar='+isVerticalScrollbar,elContentScroller?.scrollHeight,elContentScroller?.clientHeight);
	$: (isHorizontalScrollbar = elContentScroller?.scrollWidth != elContentScroller?.clientWidth),
		config; //, console.log('isHorizontalScrollbar='+isHorizontalScrollbar);
		...
		for (const clipboardItem of clipboardItems) {
			for (const type of clipboardItem.types) {
				...
				if (type.startsWith('text/html')) {
					...
					if (
						strHTML.substring(0, 26) == '<google-sheets-html-origin' ||
	...
	function pasteCells(pasteType) {
		...
		if (selCells.row == selCells.row2 && selCells.col == selCells.col2) {
			//  ...then copy the whole of the copyBuff
				...
				if (data[destR] && data[destR][destC]) {
					...
					if (pasteType.search('DISPLAY') > -1) {
						data[destR][destC].value =
							data[destR][destC].result =
							data[destR][destC].display =
								srcCell.display;
					...
					if (pasteType.search('FORMAT') > -1 && srcCell.format)
						data[destR][destC].format = JSON.parse(JSON.stringify(srcCell.format));
			...
			//  ...else paste as much of the copyBuff into the selCells as possible - repeating the copyBuff if necessary
					...
					if (data[destR] && data[destR][destC]) {
						...
						if (pasteType.search('DISPLAY') > -1) {
							...
							if (val[0] == '=') {
								...
								val = doOffsetCellRefs(
									val,
									rowDiff + srcRowOffset,
									colDiff + srcColOffset,
									copyBuff
								);
							}
							data[destR][destC].value =
	...
	function ensureDataExists(destR, destC) {
		...
		if (!config.columnSetting[destC])
			config.columnSetting[destC] = { width: 80, show: true, resizable: true };
		if (!config.rowSetting[destR])
			config.rowSetting[destR] = { height: 24, show: true, resizable: true };
	}
	...
	function deleteCells() {
		...
		for (let r = minSelRow(config); r <= maxSelRow(config); r++) {
			for (let c = minSelCol(config); c <= maxSelCol(config); c++) {
				...
				if (!data[r][c].format) data[r][c].format = JSON.parse(JSON.stringify(defaultCellFormat)); //	TODO QUESTION? Should we delete the format? NO
			}
		}
		dispatch('endedit', selCells);
	}
		...
		if (e.ctrlKey) {
			switch (e.key) {
				...
				case 'v': //	86:	^V		paste
					if (!isActionOK('PASTE')) return false;
					if (e.shiftKey)
						pasteFromClipboard('DISPLAY FORMAT'); //	paste DISPLAY (i.e. whatever is displayed, and NOT the formula)
	...
	function cellNavKeyDown(e) {
		...
		switch (e.which) {
			case 9: //	TAB
				let left = 0,
					right = config.columnSetting.length,
					top = 0,
					bottom = config.rowSetting.length;
				if (selCells.row != selCells.row2 || selCells.col != selCells.col2) {
					//	if multiple cells selected
				...
				} else if (currCell.col < right - 1) {
					//	goto next cell to the right
					currCell.col++;
				} else {
					//	go to next row (or if on bottom row gotoo cell[0][0])
					currCell.col = left;
	...
	function findNextFullCell(rowInc, colInc) {
		...
		if (rowInc) {
			...
			if (startCellEmpty || nextCellEmpty)
				//	go to next full cell
				while (idx >= 1 && idx < config.rowSetting.length - 1 && cellIsEmpty(idx, selCells.col))
					idx += rowInc;
			else {
				//	goto next full cell
				while (idx >= 0 && idx < config.rowSetting.length - 0 && !cellIsEmpty(idx, selCells.col))
					idx += rowInc;
				idx -= rowInc;
			}
		} else {
			...
			if (startCellEmpty || nextCellEmpty)
				while (idx >= 1 && idx < config.columnSetting.length - 1 && cellIsEmpty(selCells.row, idx))
					idx += colInc;
			else {
				while (idx >= 0 && idx < config.columnSetting.length - 0 && !cellIsEmpty(selCells.row, idx))
					idx += colInc;
				idx -= colInc;
			}
		}
	...
	function handleInputKeypress(e) {
		...
		if (
			...
			//	ENTER or F2 or ESC or TAB or UP or DOWN
			if (e.which != 27) {
				//	If ESC then DON'T update value
...
<div class="table">
	...
	<div class="columnHeaderWrapper">
		<div
			class="columnHeader {isHorizontalScrollbar && isVerticalScrollbar ? '' : 'span2Cols'}"
			bind:this={elColHeader}
		>
			{#each config.columnSetting as col, colIdx (colIdx)}
				...
				<div class="th resizableHeader" style={'min-width:' + col?.width + 'px'}>
					...
					{#if col?.resizable}
						<div
							class="colResizer {isSelected ? 'colSel' : ''} "
							on:mousedown|stopPropagation|preventDefault={(e) =>
								resizemousedown(e, 'COLUMN', colIdx)}
						/>
					{/if}
				</div>
			{/each}
	...
	<div class="rowHeaderWrapper">
		<div
			class="rowHeaderContainer {isHorizontalScrollbar ? '' : 'span2Rows'}"
			bind:this={elRowHeader}
		>
			{#each config.rowSetting as rowSetting, rowIdx (rowIdx)}
				...
				<div class="tr" style="height:{config.rowSetting[rowIdx]?.height}px;">
					<div
						class="rowHeader {isRowSelected ? 'rowSel' : ''}"
						on:mousedown|stopPropagation|preventDefault
						on:click={(e) => clickRowHeader(e, rowIdx)}
						on:keydown
					>
						<div class="rowNumber">{rowIdx + 1}</div>
						{#if config.rowSetting[rowIdx]?.resizable}
							<div
								class="rowResizer {isRowSelected ? 'colSel' : ''} "
		...
		>
			{#each config.rowSetting as rowSetting, rowIdx (rowIdx)}
				...
				<div class="tr" style="height:{config.rowSetting[rowIdx]?.height}px;">
					{#each config.columnSetting as col, colIdx (colIdx)}
						{@const fmt =
							data[rowIdx] && data[rowIdx][colIdx]?.format
								? data[rowIdx][colIdx].format
								: config.defaultFormat}
						{@const isSelected =
							rowIdx >= minSelRow(config) &&
							rowIdx <= maxSelRow(config) &&
							colIdx >= minSelCol(config) &&
							colIdx <= maxSelCol(config)}
						<!-- tabindex={-1} -->
						<div
							id={uid + ':' + makeStringRefFromRowCol(rowIdx, colIdx)}
							class="td {isSelected ? 'tdSelected' : ''} {rowIdx == currCell.row &&
							colIdx == currCell.col
								? 'tdFirstSelected'
								: ''} {fmt.underline ? 'underline' : ''} {fmt.italics ? 'italics' : ''} {fmt.bold
								? 'bold'
								: ''} td{fmt.align}  {fmt.border[0] != ' ' ? 'borderTop' : ''} {fmt.border[1] != ' '
								? 'borderLeft'
								: ''} {fmt.border[2] != ' ' ? 'borderBottom' : ''} {fmt.border[3] != ' '
								? 'borderRight'
								: ''}"
							style="font-size:{fmt.fontsize}px; color:{fmt.color};{isSelected
								? ''
								: 'background:' + fmt.background + ';'} width:{config.columnSetting[colIdx]
								?.width}px;"
						...
						>
							...
							{#if data[rowIdx] && data[rowIdx][colIdx]?.display != undefined}
								{@html data[rowIdx][colIdx].display != null
									? data[rowIdx][colIdx].display
